...
class Sprog extends CI_Controller {
  ...
  function profile()
  {
	...
	$config['total_rows'] = $this->db->get_where('fb_update', array('username' => $username))->num_rows();
	$config['per_page'] = 15;
	...
	$facebook_id = $this->facebook_model->get_facebook_userid($username);
	if($has_facebook_permissions && $facebook_id) {
		$data['facebook_url'] = 'https://www.facebook.com/profile.php?id='.$facebook_id;
		$data['facebook_image'] = '<img src="https://graph.facebook.com/'.$facebook_id.'/picture" width=\"50px\" height=\"50px\">';
	} else {
		$data['facebook_url'] = null;
		$data['facebook_images'] = null;
	}
  ...
  function latest()
  {
	...
	$config['total_rows'] = $this->db->get('fb_update')->num_rows();
	$config['per_page'] = 15;
	$config['full_tag_open'] = '<div id="pagination">';
	$config['full_tag_close'] = '</div>';
	$this->pagination->initialize($config);
...
//    $data['error'] = ($error =='error' ? '入力したユーザ名かパスワードが正しくありません。もう一度入力してください。' : false);
...
//      $data['has_twitter'] = 'Twitterにサインインしてください。あなたのアカウントとリンクするためにログインするか、またはSprogアカウントを作成することで利用できます。これは一度だけ実行すればリンクされます。';
    }
    if($this->facebook_model->get_user()) {
      $data['has_facebook'] = 'You are signed in with Facebook, but you must log in or register with us to link accounts. You will only have to do this once.';
//      $data['has_facebook'] = 'Facebookにサインインしてください。あなたのアカウントとリンクするためにログインするか、またはSprogアカウントを作成することで利用できます。これは一度だけ実行すればリンクされます。';
    }
...
//      }
  ...
  function view_comment()
  {
	...
	$config['total_rows'] = $this->db->get_where('fb_comment', array('update_id' => $update_id))->num_rows();
	$config['per_page'] = 15;
	$config['full_tag_open'] = '<div id="pagination">';
	$config['full_tag_close'] = '</div>';
	$this->pagination->initialize($config);
