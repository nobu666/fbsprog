...
class Facebook_Model extends CI_Model {
  ...
  function remove()
  {
    ...
    if (!empty($facebook_parameters) && $facebook->fb_params['uninstall'] == 1) {
      $this->db->delete('fb_facebook', array('id' => $facebook->fb_params['user']));
    }
  }
  ...
  {
    $this->db->select('*')->from('fb_user')->join('fb_facebook', 'fb_facebook.user_username = fb_user.username', 'left');
  ...
  {
    $query = $this->db->get_where('fb_facebook', array('id' => $facebook_id));
    if($query->num_rows() == 0) {
      $user = array(
        'id' => $facebook_id,
        'session_key' => '',
      ...
      $query = $this->db->insert('fb_facebook', $user);
      return true;
    } else {
      return false;
    }
  ...
  {
    $query = $this->db->get_where('fb_facebook', array('user_username' => $username));
  ...
  function has_permissions($username)
  {
      ...
      $query = $this->db->get_where('fb_facebook', array('user_username' => $username, 'session_key !=' => ''));
  ...
  {
  	$query = $this->db->get_where('fb_update', array('id' => $update_id, 'facebook_id !=' => ''));	
  	return ($query->num_rows() == 1 ? $query->row()->facebook_id : false);
  }

  function get_facebook_userid($user_username)
  {
  	$query = $this->db->get_where('fb_facebook', array('user_username' => $user_username, 'id !=' => ''));	
